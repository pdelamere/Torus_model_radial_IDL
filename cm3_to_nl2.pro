;------------------------------------------------------------------
pro get_NL2,nr,tr,nl2,nl2e,hr,nar
;------------------------------------------------------------------
@common
...
nar = replicate({dens_lat, nsp: 0.0, ns2p: 0.0, ns3p: 0.0, ns4p: 0.0, nop: 0.0, $
                no2p: 0.0, nel: 0.0, nelh: 0.0, ns: 0.0, no: 0.0},max_theta*2+1)
...
for ii = 0,n_elements(nr)-1 do begin

   hr(ii).sp = sqrt(2.0*Tr(ii).Tsp*1.6e-19*(1+Tr(ii).Tel/Tr(ii).Tsp)/(3.0*ms*omega^2))/1e3
   hr(ii).s2p = sqrt(2.0*Tr(ii).Ts2p*1.6e-19*(1+2.0*Tr(ii).Tel/Tr(ii).Ts2p)/(3.0*ms*omega^2))/1e3
   hr(ii).s3p = sqrt(2.0*Tr(ii).Ts3p*1.6e-19*(1+3.0*Tr(ii).Tel/Tr(ii).Ts3p)/(3.0*ms*omega^2))/1e3
   hr(ii).op = sqrt(2.0*Tr(ii).Top*1.6e-19*(1+Tr(ii).Tel/Tr(ii).Top)/(3.0*mo*omega^2))/1e3
   hr(ii).o2p = sqrt(2.0*Tr(ii).To2p*1.6e-19*(1+2.0*Tr(ii).Tel/Tr(ii).To2p)/(3.0*mo*omega^2))/1e3

   T = tr(ii)
   n = nr(ii)
   h = hr(ii)

;   print,'hr.sp 1...',hr(ii).sp,Lshell(ii)

   i = ii+bufcel


   Lshl = Lshell(ii)
;   cm3_expand,n,narr,T,nar,max_theta,Lshl         ;this version determines diffusive equilibrium 
   cm3_expand_1,n,T,h,nar,max_theta,Lshl,s    ;this version uses the analytical form for scale height
   ...
   ;print,'nl2 sp...',nl2(i).nop,h.op,Lshl   
...
;Rj = 7.14e4
;;!p.multi=[1,1,1]
;plot,nar.nel,s/Rj,/xlog,yrange=[-4,4],xrange=[0.1,3000]
...
;nl2b= [nl2(0).nsp,nl2.nsp,1.5*nl2(nbox-1).nsp - 0.5*nl2(nbox-2).nsp]
nl2b= [nl2(0).nsp,nl2.nsp,4e33]
...
;nl2b = [nl2(0).ns2p,nl2.ns2p,nl2(nbox-1).ns2p]
nl2b = [nl2(0).ns2p,nl2.ns2p,2e34]
gradnl2 = (shift(nl2b,-1) - nl2b)/(shift(L,-1)-L)
flux = (dll/Lp^2)*gradnl2
dL_flx = L_flx - shift(L_flx,1) 
dL_flx(0) = dL_flx(1)
...
;nl2b = [nl2(0).ns3p,nl2.ns3p,nl2(nbox-1).ns3p]
nl2b = [nl2(0).ns3p,nl2.ns3p,2e34]
gradnl2 = (shift(nl2b,-1) - nl2b)/(shift(L,-1)-L)
flux = (dll/Lp^2)*gradnl2
dL_flx = L_flx - shift(L_flx,1) 
dL_flx(0) = dL_flx(1)
...
;nl2b = [nl2(0).ns4p,nl2.ns4p,nl2(nbox-1).ns4p]
nl2b = [nl2(0).ns4p,nl2.ns4p,1e34]
gradnl2 = (shift(nl2b,-1) - nl2b)/(shift(L,-1)-L)
flux = (dll/Lp^2)*gradnl2
dL_flx = L_flx - shift(L_flx,1) 
dL_flx(0) = dL_flx(1)
...
;nl2b = [nl2(0).nop,nl2.nop,nl2(nbox-1).nop]
nl2b = [nl2(0).nop,nl2.nop,1e35]
gradnl2 = (shift(nl2b,-1) - nl2b)/(shift(L,-1)-L)
flux = (dll/Lp^2)*gradnl2
dL_flx = L_flx - shift(L_flx,1) 
dL_flx(0) = dL_flx(1)
...
;nl2b = [nl2(0).no2p,nl2.no2p,nl2(nbox-1).no2p]
nl2b = [nl2(0).no2p,nl2.no2p,1e35]
gradnl2 = (shift(nl2b,-1) - nl2b)/(shift(L,-1)-L)
flux = (dll/Lp^2)*gradnl2
dL_flx = L_flx - shift(L_flx,1) 
dL_flx(0) = dL_flx(1)
...
;------------------------------------------------------------------
pro transport_NL2_grad_tot,nl2,nl2e,DLL_0,DLL_alpha               
;pro transport_NL2,nl2,nl2e
;------------------------------------------------------------------
@common
;@params

nl2tot = nl2.nsp+nl2.ns2p+nl2.ns3p+nl2.nop+nl2.no2p
...
;nl2b = [nl2(0).no2p,nl2.no2p,nl2(nbox-1).no2p]
nl2bno2p = [nl2(0).no2p,nl2.no2p,3e35]
nl2b = [nl2tot(0),nl2tot,1e35]
gradnl2 = (shift(nl2b,-1) - nl2b)/(shift(L,-1)-L)
flux = (dll/Lp^2)*gradnl2
dL_flx = L_flx - shift(L_flx,1) 
dL_flx(0) = dL_flx(1)
nl2bno2p = nl2bno2p + L^2*(dt_trans/dL_flx)*(flux-shift(flux,1))
nl2.no2p = nl2bno2p(1:nintrvl)
...
;------------------------------------------------------------------
pro iterate_NL2_to_equator,nl2,nl2e,nr,tr,hr
;------------------------------------------------------------------
@common
...
get_NL2,np,tr,nl2_p,nl2e_p,hr
get_NL2,nm,tr,nl2_m,nl2e_m,hr
