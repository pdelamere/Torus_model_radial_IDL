...
;   print,'vwght...',i,theta_G*!radeg,vwght(i)
   ...
   for j = 0,it-1 do begin
      ...
      n1 = [n1_elec,n1_ions]
;      print,'n1...',n1(0),n1(1:*),total(n1(1:*))
      f = total([felec,fions])
      df = total([dfelec,dfions])
;      print,f

;      f = double(total(Z*n1))
;      df = double(total(Z*Z*n1*anis/Tarr))
      if (abs(f) lt fmin) then goto, JUMP
      
;      if (df lt 0.01) then begin
;         print,'Error...will not converge',Tkappa
;         goto, JUMP       
;      endif
      phi = phi-f/df
...
;   sclhgt,s3,L,theta_G_0,theta_G,Z,A,Tarr,anis,n0,n1,fmin
;   sclhgt_kappa,s3,L,theta_G_0,theta_G,Z,A,Tarr,kappa,n0,nk,fmin,Tk
   narr(*,i) = n1
;   nkappa(*,i) = nk
   Tkappa(i) = Tk

endfor
;   print,'Tkappa...',Tkappa(30)
...
;plot_oi,nkappa(5,*),max_theta-findgen(2.0*max_theta),$
;    yrange=[-max_theta,max_theta],/ysty,$
;    title='System III Long = '+strtrim(string(s3*!radeg),2),$
;    ytitle = 'Jovigraphic latitude',xtitle='density (cm!u-3!n)',$
;    xrange=[1,3.0*n0(5)],/xsty
;oplot,nkappa(0,*),max_theta-findgen(2.0*max_theta),linestyle=1
;oplot,nkappa(1,*),max_theta-findgen(2.0*max_theta),linestyle=2
;oplot,nkappa(2,*),max_theta-findgen(2.0*max_theta),linestyle=3
;oplot,nkappa(3,*),max_theta-findgen(2.0*max_theta),linestyle=4
;oplot,nkappa(4,*),max_theta-findgen(2.0*max_theta),linestyle=5
...
;plot_oi,Tkappa(5,*),max_theta-findgen(2.0*max_theta),$
;    yrange=[-max_theta,max_theta],/ysty,$
;    title='System III Long = '+strtrim(string(s3*!radeg),2),$
;    ytitle = 'Jovigraphic latitude',xtitle='Temperature (eV)',$
;    xrange=[1,1000],/xsty
;oplot,Tkappa(0,*),max_theta-findgen(2.0*max_theta),linestyle=1
;oplot,Tkappa(1,*),max_theta-findgen(2.0*max_theta),linestyle=2
;oplot,Tkappa(2,*),max_theta-findgen(2.0*max_theta),linestyle=3
;oplot,Tkappa(3,*),max_theta-findgen(2.0*max_theta),linestyle=4
;oplot,Tkappa(4,*),max_theta-findgen(2.0*max_theta),linestyle=5
