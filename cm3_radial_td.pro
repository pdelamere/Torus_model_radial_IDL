...
net_source_arr(whge) = fextend*net_source_0*(Lshell(whge)/6.0)^(-net_source_alpha)
...
nr(whhot1).nsph = 0.0001 ;(0.1/1.5)*(Lshell(whhot1)-6.0)
nr(whhot1).noph = 0.0001 ; (0.05/1.5)*(Lshell(whhot1)-6.0)
;nr.nsph = 0.0
;nr.noph = 0.0
...
for j = 0,nit do begin
    ...
    runtime = j*runt/8.64e4
    print,'time...',j*runt/60./60./24.,runtime

    ft1 = exp(-(runtime-40)^2/22.5^2)
;    net_source_0 = net_source + No*ft1
;    flocal = flocal_0 + 3.0*flocal_0*ft1
    net_source_0 = net_source + No*ft1
    net_source_arr(whge) = fextend*net_source_0*(Lshell(whge)/6.0)^(-net_source_alpha)
    net_source_arr(0) = net_source_arr(0) + flocal*net_source_0
;    dLL_0 = dll_init + dll_init*2.0*ft1
...
for ii = 0,nbox-1 do begin
    ...
    print,'box...',i+1,net_source_arr(ii),dLL_0
...
for k = 0,nit_trans-1 do begin
   transport_NL2_td,nl2,nl2e,runtime,ft1
endfor

tr.Tsp = nl2e.nsp/(nl2.nsp*Lshell^(8./3.)*Lshell^2)
tr.Ts2p = nl2e.ns2p/(nl2.ns2p*Lshell^(8./3.)*Lshell^2)
tr.Ts3p = nl2e.ns3p/(nl2.ns3p*Lshell^(8./3.)*Lshell^2)
tr.Top = nl2e.nop/(nl2.nop*Lshell^(8./3.)*Lshell^2)
tr.To2p = nl2e.no2p/(nl2.no2p*Lshell^(8./3.)*Lshell^2)
...
save,filename='restart.sav',nr,tr,nl2,nl2e,mrr,s,tot_peuv,tot_peuvh
