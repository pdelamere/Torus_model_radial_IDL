...
@common
      ...
      plot_io,tm,temps.Tsp,/nodata,title='Temperature',$
         ...
         yrange=[1.0,1000.0],/ysty,xrange=[0,dt*ntm]/8.64e4,/xsty
      ...
      legend,['S!u+!n','S!u++!n','S!u+++!n','O!u+!n','O!u++!n','T!de!n'],$
       linestyle=[0,0,0,0,0,0],colors=[10,75,50,150,200,100],/top,$
       /right
      endif
...
@common
      ...
      plot_io,tm,dens.nel,/nodata,title='Density',$
         ...
         yrange=[10.0,10000.0],/ysty,xrange=[0,dt*ntm]/8.64e4,/xsty
...
;      endif
      vol = 3.3e31
      plot,tm,p.lps2p*vol*1.6e-19,xtitle='time (days) ',$
              ytitle='Power Radiated (Terawatts)',$
              xrange=[0,dt*ntm]/8.64e4,/xsty
      oplot,tm,p.lpsp*vol*1.6e-19,linestyle=1 
      oplot,tm,p.lps3p*vol*1.6e-19,linestyle=2 
      oplot,tm,p.lpop*vol*1.6e-19,linestyle=4 
;      oplot,tm,p.po2p,linestyle=5 
      if (lonoff eq 1.0) then begin
      legend,['S!u+!n','S!u++!n','S!u+++!n','O!u+!n'],$
       linestyle=[1,0,2,4],/top,/left
      endif
      end;RADPOWER
...
lct = widget_base(lc,/frame,/column,space=10)
...
;draw_citep = widget_draw(rc,xsize=108,ysize=18)
...
;xmenu,['< 2x','> 2x'],lct,/column,/exclusive,$
;      space=1,uvalue=['DTUP','DTDOWN'],title='time step'
...
;w1 = widget_label(lcchem,value = 'Chemistry')

;xmenu,['on','off'],lcchem,/row,/exclusive,$
;      space=1,uvalue=['CX15ON','CX15OFF'],title='CX15'

;w1 = widget_droplist(lcchem,value=['on','off'],uvalue=['CX15ON','CX15OFF'],$
;      title='CX15')
;widget_control,wrt,set_value=runt/8.64e4
...
;t1 = WIDGET_TEXT(lc, xsize=20, ysize=10, /SCROLL, $
;      value=[ 'Mixing Ratios'])
...
;w1 = widget_button(rc,value = ' PostScript ',uvalue = 'PS')
...
draw = widget_draw(mc,xsize=650,ysize=500)
...
xmenu,['on','off'],trc,/row,/exclusive,$
      space=1,uvalue=['CX16ON','CX16OFF'],title='k16'
...
xyouts,0.1,0.15,'k16:  S!u3+!n + S!u+!n --> S!u++!n + S!u++!n  (3.6e-10)',/normal
